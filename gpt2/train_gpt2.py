...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        B, T = idx.size()
        assert T <= self.config.block_size, f"Too long message, should be less or equal than {self.config.block_size}"
...
B = 16
T = 1024
assert total_batch_size % (B*T * ddp_world_size) == 0, "total_batch_size should be divisible by B * T"
...
for step in range(max_steps):
    ...
    if master_process and (step % 500 == 0 or last_step):
        model.eval()
